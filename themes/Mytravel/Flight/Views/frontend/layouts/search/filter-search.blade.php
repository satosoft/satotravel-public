        ...
        <form action="{{url(app_get_locale(false,false,'/').config('flight.flight_route_prefix'))}}" class="bravo_form_filter">
            ...
            <div class="sidenav border border-color-8 rounded-xs">
                ...
                <div id="bravo-filter-price" class="accordion rounded shadow-none bravo-filter-price">
                    <?php
                    $price_min = $pri_from = floor(App\Lib\Currency::convertPrice($flight_min_max_price[0]));
                    $price_max = $pri_to = ceil(App\Lib\Currency::convertPrice($flight_min_max_price[1]));
                    if (!empty($price_range = Request::query('price_range'))) {
                        $pri_from = explode(";", $price_range)[0];
                        $pri_to = explode(";", $price_range)[1];
                    }
                    $currency = App\Lib\Currency::getCurrency(App\Lib\Currency::getCurrent());
                    ?>
                    <div class="border-0">
